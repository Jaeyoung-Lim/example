# RLtools Example: Implementing Custom Environments

This repository contains two custom environment examples:

1. **Pendulum Environment** (`include/my_pendulum`): A simple pendulum swing-up task (191 lines of code)
2. **Fixed-Wing Aircraft Environment** (`include/fixedwing`): A velocity reference tracking task for fixed-wing aircraft
## UNIX (Linux / macOS)
Dependencies
```
sudo apt install git cmake build-essential
```
Optional dependencies (Python for the experiment tracking UI, OpenBLAS for acceleration and protobuf for Tensorboard logging)
```
sudo apt install python3 libopenblas-dev protobuf-compiler libprotobuf-dev
```
```
git clone https://github.com/rl-tools/example --recursive
cd example
cmake . -B build -DCMAKE_BUILD_TYPE=Release -DRL_TOOLS_DISABLE_TENSORBOARD=ON
cmake --build build --target my_pendulum -j$(nproc)
./build/my_pendulum
ln -sf ../../experiments external/rl_tools/experiments  # Create symlink for web access
./external/rl_tools/tools/serve.sh ./external/rl_tools ./experiments
```
Navigate to [http://localhost:8000/static/extrack_ui](http://localhost:8000/static/extrack_ui) to see the experiment tracking UI.

To train the fixed-wing aircraft environment instead:
```
cmake --build build --target fixedwing_training -j$(nproc)
./build/fixedwing_training
```
...
The experiment tracking and save-trajectories step will periodically record trajectories and store them as `.json.gz` files. After/while running the training, create a symlink with `ln -sf ../../experiments external/rl_tools/experiments` (if not done already) and run `./external/rl_tools/tools/serve.sh ./external/rl_tools ./experiments` which will start a local webserver. Navigate to [http://localhost:8000/static/extrack_ui](http://localhost:8000/static/extrack_ui) to see the recorded trajectories and visualizations based on the render function you provided.